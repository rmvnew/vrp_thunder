...
RegisterNUICallback('complete',function(data)
	if fuelTotal < 99.0 then
		if vSERVER.checkPayment((100.0 - fuelTotal)*Config.tax) then
			state = 1
...
AddEventHandler('pedReturn', function(index, x, y, z)
    -- Verifica se o ped na posição 'index' existe na tabela
    if peds[index] and peds[index].ped then
        local ped = peds[index].ped
        
        -- Descongela a posição da entidade do ped
        FreezeEntityPosition(ped, false)
        
        -- Faz o ped andar até a coordenada (x, y, z) com velocidade 1.6
        TaskGoToCoordAnyMeans(ped, x, y, z, 1.6, 0, 0, 786603, 1.0)
        
        -- Esperar 1 segundo para verificar se o ped está em serviço
        SetTimeout(1000, function()
            -- Verifica se o ped ainda está em serviço
            if not vSERVER.getService(ped) then
                -- Se não estiver em serviço, sincroniza a animação (opcional)
                -- vSERVER.syncAnim(ped, true, 'freeze')
            end
        end)
    else
        -- Mensagem de erro opcional se o ped não for encontrado
        print("Erro: Ped não encontrado no índice " .. tostring(index))
    end
end)
		...
		if GetSelectedPedWeapon(ped) == 883325847 and not isFueling then
			...
			if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(GetEntityCoords(ped), vehicleCoords) < 2.5 then
				if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
					local jerrycanAmmo = GetAmmoInPedWeapon(ped, 883325847)
					local vehicleFuel = GetVehicleFuelLevel(vehicle)
			
					if jerrycanAmmo > 100 then
						if vehicleFuel < 99.0 then
							if openVeh[vehicle] then
								DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 0.5, '🛢️ ~b~'..Round(jerrycanAmmo / 4500 * 100, 1)..'%~w~    ⛽ ~p~'..Round(vehicleFuel, 1)..'%')
								...
								if IsControlJustPressed(0, 24) then
									...
									if vehicleHandle ~= 0 and IsEntityAVehicle(vehicleHandle) and openVeh[vehicleHandle] then
										vehicle = vehicleHandle
										local currentFuel = vehicleFuel
										jerrycan = true
			
										if currentFuel < 99.0 then
											while not IsControlJustReleased(0, 24) and jerrycan and openVeh[vehicle] do
												if not bool then
													jerrycan = true
												...
												DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 0.5, '🛢️ ~b~'..Round(jerrycanAmmo / 4500 * 100, 1)..'%~w~    ⛽ ~p~'..Round(currentFuel, 1)..'%')
			
												local fuelToAdd = math.random(2, 3) / 100.0
			
												if jerrycanAmmo - fuelToAdd * 100 >= 0 then
													currentFuel = currentFuel + fuelToAdd
													SetPedAmmo(ped, 883325847, math.floor(jerrycanAmmo - fuelToAdd * 100))
													jerrycanAmmo = GetAmmoInPedWeapon(ped, 883325847)
												else
													jerrycan = false
												end
												...
												vSERVER.syncFuel(VehToNet(vehicle), currentFuel) -- Sincronização incremental do combustível
												
												Citizen.Wait(5)
											end
											
											-- Parar a animação e limpar o estado
											if bool then
								...
								DrawText3Ds(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 0.5, '🔒~g~[G]~w~')
							end
