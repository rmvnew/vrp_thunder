...
function tvRP.openOrgMenu()
    ...
    if vRP.hasPermission(user_id, "perm.policia") or vRP.hasPermission(user_id,"perm.policiacivil") or vRP.hasPermission(user_id,"perm.swat") or vRP.hasPermission(user_id,"perm.lspd") then
        if vRP.checkPatrulhamento(user_id) then
            local menudata = vRP.buildMenu("police_menu", {player = source})
            menudata.name = "POLICIA"
            vRP.openMenu(source,menudata)
        else
            TriggerClientEvent("Notify",source,"negado","Você precisa bater o ponto", 5)
        end
