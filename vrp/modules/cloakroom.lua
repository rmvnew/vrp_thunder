...
local function build_client_points(source)
    for k, v in pairs(cfg.cloakrooms) do
        ...
        if cloakroom and menu then
            ...
            local function cloakroom_enter(source, area)
                ...
                if user_id and vRP.hasPermissions(user_id, gcfg.permissions or {}) then
                    if gcfg.not_uniform then
                        local data = vRP.getUserDataTable(user_id)
                        -- Perform any operations with `data` if necessary
                    end
            ...
            -- Adding a marker for the cloakroom
            vRPclient._addMarker(source, 21, x, y, z - 0.6, 0.5, 0.5, 0.4, 128, 0, 128, 75, 100)

            -- Setting up the named area using vRPclient.setNamedArea
            vRPclient.setNamedArea(source, "vRP:cfg:cloakroom" .. k, x, y, z, 2, 2, cloakroom_enter, cloakroom_leave)
        end
    end
end
