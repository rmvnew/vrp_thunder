...
addMod = function(vehicle, modId, stock, modPrice)
    ...
    elseif modId == "vehicle_traction" then
        ...
        for value, vehd in pairs(vehData) do
            if value == currentModIndex..":"..Model then
...
loadMods = function(vehicle)
    ...
    SetNuiFocus(true, true)  -- Ativar NUI Focus no início
    return modMenu
end
...
RegisterNUICallback("buyComponent", function(data, cbj)
    ...
    local result = vSERVER.buyComponent({ price = data.component.price, currentMechanic = currentMechanic, model = Model, plate = vehPlate}, getVehicleProperties(vehicle))
    if result then
...
RegisterNUICallback("applyFitment", function(dt, cbj)
    ...
    local vehPlate = trim(GetVehicleNumberPlateText(vehicle))
    local Model = GetEntityArchetypeName(vehicle)
    local result = vSERVER.buyComponent({ price = Config.Menus.fitment.price, currentMechanic = currentMechanic, model = Model, plate = vehPlate}, getVehicleProperties(vehicle))
    if result then
        if not result.status then
            return cbj(false)
        end
...
RegisterNUICallback("buyColor", function(data, cbj)
    ...
    local result = vSERVER.buyComponent({ price = colorPrice, currentMechanic = currentMechanic, model = Model, plate = vehPlate}, getVehicleProperties(vehicle))
    if result then
        if not result.status then
            return cbj(false)
        end
...
RegisterNUICallback("buyBasket", function(data,cb)
    ...
    local result = vSERVER.buyBasketData({data.type, data.basketdata, currentMechanic,vehPlate,Model}, getVehicleProperties(vehicle))
    if result then
    ...
    while true do
        ...
        if next(vehData) then
            if IsPedInAnyVehicle(playerPed) and not menuOpened then
                ...
                local Model = GetEntityArchetypeName(pVehicle)
                if vehData[vehPlate..":"..Model] then
                    sleep = 1
                    if lastVehicle ~= pVehicle then
                        lastVehicle = pVehicle
                        setVehicleModification(pVehicle, vehPlate)
                    end
                    if vehData[vehPlate..":"..Model].tuner_chip and vehData[vehPlate..":"..Model].tuner_chip == "drift" then
                    ...
                    if vehData[vehPlate..":"..Model].nitro and vehData[vehPlate..":"..Model].nitro == "nos" then
                        if IsControlJustPressed(0, Config.Settings.nitro.keyCode) and not nitroBoosted then
                            nitroBoosted = true
                            useNitro(pVehicle)
                        end
                    end
                    if vehData[vehPlate..":"..Model].popcorn and vehData[vehPlate..":"..Model].popcorn == "popcorn" then
            ...
            if Config.Settings.syncFitment and not menuOpened then
                for pVehicle in EnumerateVehicles() do 
                    local vehPlate = trim(GetVehicleNumberPlateText(pVehicle))
                    local Model = GetEntityArchetypeName(pVehicle)
                    if vehData[vehPlate..":"..Model] and vehData[vehPlate..":"..Model].fitment then
                        for ftype, value in pairs(vehData[vehPlate..":"..Model].fitment) do
                            if value == -1 then
                                value = -1.00
                            elseif value == 1 then
                                value = 1.00
