  ...
  ot = 1000
		...
		if IsPedInAnyVehicle(ped) then
			...
			if GetPedInVehicleSeat(veh,-1) == ped then
				DisableControlAction(0,454545,true)
				DisableControlAction(0,45454,true)
				...
				if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
					...
					if count_bcast_timer > delay_bcast_timer then
						...
						if GetVehicleClass(veh) == 18 then
							...
							if (dsrn_mute ~= cache_mute) then
                                TriggerServerEvent("lvc_TogDfltSrnMuted_s",dsrn_mute,playerAround)
                                cache_mute = dsrn_mute
                            end
                            if (state_lxsiren[veh] ~= cache_lxsiren[veh]) then
                                TriggerServerEvent("lvc_SetLxSirenState_s",state_lxsiren[veh],playerAround)
                                cache_lxsiren[veh] = state_lxsiren[veh]
                            end
                            if (state_airmanu[veh] ~= cache_airmanu[veh]) then
                                TriggerServerEvent("lvc_SetAirManuState_s",state_airmanu[veh],playerAround)
                                cache_airmanu[veh] = state_airmanu[veh]
                            end
						end
						if (state_indic[veh] ~= cache_indic[veh]) then
                            TriggerServerEvent("lvc_TogIndicState_s",state_indic[veh],playerAround)
                            cache_indic[veh] = state_indic[veh]
                        end
					else
